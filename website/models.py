...
class Note(db.Model):
    ...
    dm_to_user_id = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=True)
    ...
    user = db.relationship("User", foreign_keys=[user_id], backref="notes")
...
class User(db.Model, UserMixin):
    ...
    channels = db.relationship(
        "Channel", backref="user", cascade="all, delete, delete-orphan", lazy="dynamic"
    )
...
class DirectMessage(db.Model):
    ...
    deleted_date = db.Column(db.DateTime(timezone=True), nullable=True)
    edited = db.Column(db.Boolean, default=False)
    edited_date = db.Column(db.DateTime(timezone=True), nullable=True)
    ...
    recipient = db.relationship(
        "User", foreign_keys=[recipient_id], backref="received_dms"
    )

    def __repr__(self):
        return f"DM: {self.id} from {self.sender_id} to {self.recipient_id}"
